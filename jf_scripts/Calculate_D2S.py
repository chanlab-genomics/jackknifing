...
def main():
	...
	parser.add_argument('--kmerset1', metavar='KmerSet1.21mers.gz', type=lambda x: check_file_exists(x), required=True, help='Kmers for dataset 1, can be gziped')
	parser.add_argument('--kmerset1_freq', metavar='KmerSet1.21mers.charFreq', type=lambda x: check_file_exists(x), required=True, help='Character frequency for dataset 1, can be gziped')

	parser.add_argument('--kmerset2', metavar='KmerSet2.21mers.gz', type=lambda x: check_file_exists(x), required=True, help='Kmer for dataset 2, can be gziped')
	parser.add_argument('--kmerset2_freq', metavar='KmerSet2.21mers.charFreq', type=lambda x: check_file_exists(x), required=True, help='Character frequency for dataset 2, can be gziped')
	
	parser.add_argument('--D2S_out', metavar='D2S.txt', type=lambda x: write_file_check_compression(x), required=False, default=sys.stdout, help='Output for D2S score (default: %(default)s)')
	...
	d2Score_kmerset1_VS_kmerset2 = calculate_D2S(args.kmerset1, args.kmerset1_freq, args.kmerset2, args.kmerset2_freq, logger)
	logger.info('kmerset1 VS. kmerset2 d2Score:%s', d2Score_kmerset1_VS_kmerset2) ## INFO
	
	d2Score_kmerset1_VS_kmerset1 = calculate_D2S(args.kmerset1, args.kmerset1_freq, args.kmerset1, args.kmerset1_freq, logger)
        logger.info('kmerset1 VS. kmerset1 d2Score:%s', d2Score_kmerset1_VS_kmerset2) ## INFO
	
	d2Score_kmerset2_VS_kmerset2 = calculate_D2S(args.kmerset2, args.kmerset2_freq, args.kmerset2, args.kmerset2_freq, logger)
        logger.info('kmerset2 VS. kmerset2 d2Score:%s', d2Score_kmerset1_VS_kmerset2) ## INFO
	
	D2S_distance = d2ScoreNormalization(d2Score_kmerset1_VS_kmerset2, d2Score_kmerset1_VS_kmerset1, d2Score_kmerset2_VS_kmerset2)
	logger.info('D2S_distance:%s', D2S_distance) ## INFO
	
	args.D2S_out.write(args.kmerset1 + ';' + args.kmerset2 + ';' + str(D2S_distance)+'\n')
...
def d2ScoreNormalization(d2Score_kmerset1_VS_kmerset2, d2Score_kmerset1_VS_kmerset1, d2Score_kmerset2_VS_kmerset2):
	...
	value = d2Score_kmerset1_VS_kmerset2 / (math.sqrt(d2Score_kmerset1_VS_kmerset1 * d2Score_kmerset2_VS_kmerset2))
...
def calculate_D2S(KmerSet1_fileName, KmerSet1_freq_fileName, KmerSet2_fileName, KmerSet2_freq_fileName, logger):
	...
	Kmer_iter = iterate_Kmer_sets(KmerSet1_fh, KmerSet2_fh, logger, Both_KmerSets=True, KmerSet1_Only=False, KmerSet2_Only=False)
	...
	if len(KmerSet1_seq) != len(KmerSet2_seq):
		logger.error('Kmer sizes are different between the two datasets: %s:%s\t%s:%s', KmerSet1_fh.name, len(KmerSet1_seq), KmerSet2_fh.name, len(KmerSet2_seq)) ## ERROR
		sys.exit(1)
	...
	logger.debug('kmerset1_NumKmers:%s\tkmerset2_NumKmers:%s', kmerset1_NumKmers, kmerset2_NumKmers) ## DEBUG
	...
	for KmerSet1_seq, KmerSet1_count, KmerSet2_seq, KmerSet2_count in Kmer_iter:
		...
		PwX=calculate_PropKmerOccurrence(KmerSet1_seq, kmerset1_freq) # Probability of k-mer occurrence in seq 1.
		PwY=calculate_PropKmerOccurrence(KmerSet2_seq, kmerset2_freq) # Probability of k-mer occurrence in seq 2.
		
		kmerScoreXBis = KmerSet1_count - (kmerset1_NumKmers*PwX)
		kmerScoreYBis = KmerSet2_count - (kmerset2_NumKmers*PwY)
		
		d2Score_tmp = (kmerScoreXBis*kmerScoreYBis) / math.sqrt(kmerScoreXBis*kmerScoreXBis + kmerScoreYBis*kmerScoreYBis)
		d2Score += (kmerScoreXBis*kmerScoreYBis) / math.sqrt(kmerScoreXBis*kmerScoreXBis + kmerScoreYBis*kmerScoreYBis)
		
		logger.debug('PwX:%s\tPwY:%s\tkmerScoreXBis:%s\tkmerScoreYBis:%s\td2Score:%s', PwX, PwY, kmerScoreXBis, kmerScoreYBis, d2Score_tmp) ## DEBUG
